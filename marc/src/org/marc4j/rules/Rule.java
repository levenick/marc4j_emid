...
//     */
//    private int metaData_fieldNumber = -1;
//    private char metaData_subfieldChar = '?';
    static int serial = 0;
    /**
...
//            return;
//        }
        createAndAddNewField(actionList, theRecord, targetField, factory);
        if (twoOrMoreFieldsToOutput()) {
            Globals.setPartNumber(-17);
            if (!needPartsFields()) {
                createAndAddNewField(actionListAdditional, theRecord, targetField, factory); // the second field
            } else {
...
//    }
    ...
    private void dealWithMultipleParts(Record theRecord, int targetField, MarcFactory factory) {
        for (int i = 1; i <= Globals.getRepetitions(); i++) {
            ...
            createAndAddNewField(actionListAdditional, theRecord, targetField, factory);
        }
    }
    ...
    private void createAndAddNewField(ActionList actionList, Record theRecord, int targetField, MarcFactory factory) {
    ...
    private void addSubfieldR(Record theRecord, DataField theField) {
        Utils.addSubField(theRecord, theField, 'r', theRuleSet.getInputFileName() + " R" + this.serialNumber);
    }
