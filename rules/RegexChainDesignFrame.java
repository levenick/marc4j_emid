...
public class RegexChainDesignFrame extends javax.swing.JFrame {
    ...
    private void lowerButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_lowerButtonActionPerformed
        ...
        while (theMarcReader.hasNext()) {
            ...
            for (String nextS : list) {
                if (regexMatchesLowercase(aRecord, nextS, theField)) {
                    ctrList.add(nextS);
                    foundOne = true;
                    break;
                }
    ...
    private boolean regexMatches(Record theRecord, String regex, FieldSpecifier theFieldSpec) {
        ...
        for (String nextData : list) {
            ...
            if (matcher.find()) {
                System.out.println("regex=" + regex + " nextData = " + nextData);
                return true;
            }
        }
   ...
   private boolean regexMatchesLowercase(Record theRecord, String regex, FieldSpecifier theFieldSpec) {
        //System.out.println("gonna match " + regex + " to " + theFieldSpec);
        int fieldNumber = theFieldSpec.getFieldNumber();
        char subFieldChar = theFieldSpec.getSubfieldChar();
        StringList list = Utils.getDataFromFieldAndSubfield(theRecord, fieldNumber, subFieldChar);
        ...
        for (String nextData : list) {
            Pattern pattern = Pattern.compile(regex);
            Matcher matcher = pattern.matcher(nextData.toLowerCase());
    ...
    private void displayResults(StringCounterList ctrList, StringList list) {
        outTA.setText("");
        for (StringCounter nextSC : ctrList) {
            nextSC.decrement();  // so the counts are right...
        }
        for (int i=0; i<list.size(); i++) {
            StringCounter nextSC = ctrList.get(i);
            String s = list.get(i);
            outTA.append(nextSC.getCount() + "\t" + s + "\n");
